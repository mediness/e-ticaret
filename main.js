...
function addToBasket(product){
    // sepette parametre olarak gelen elemanı aktar
    const foundItem= basket.find((basketItem)=>basketItem.id === product.id)
    
    if(foundItem){
        // eğer elemandan sepette varsa bulunan elemanın miktarnı arttır
        foundItem.amount ++
    } else{
...
function addList(){
    ...
    listItem.innerHTML = `
                        ...
                        <button id="del" onclick="deleteItem({id: ${product.id}, price:${product.price}, amount:${product.amount}})"> SİL </button>
...
function deleteItem(deletingItem){
    // id si silinecek elemanın id siyle eşit olmayanları al
